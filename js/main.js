...
async function generatenfts(){
    ...
    firebaseRef.on("value", function(data){
        ...
        `
        document.querySelector(".headingdiv").appendChild(newdiv);
    });
    await propertynfts.forEach(async function(e){
        var img = "";
        firebaseRef = firebase.database().ref("Users/"+accountID+"/NFTs/"+e);
        firebaseRef.child("img").once("value",async function(data){
          await firebase.storage().ref(data.val()).getDownloadURL().then(imgUrl => {
            img = imgUrl;
          });
          createNFTcard(img,e);
          ...
          [...document.getElementsByClassName("card")].forEach(function(e){
            ...
            var counter = 0;
            [...document.querySelectorAll(".reveal-box")].forEach(function(e){
                setTimeout(function(){
...
function createNFTcard (img,e) {
    ...
    `
    document.querySelector(".carddiv").appendChild(newdiv);
}
